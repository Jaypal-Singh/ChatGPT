...
//             className={`group rounded-xl p-4 cursor-pointer transition-all duration-200 border ${
//               activeId === chat.id
...
//                   activeId === chat.id ? "text-slate-800" : "text-gray-200"
//                 }`}
//               >
//                 {chat.title}
//               </h3>
//               <div
//                 className={`flex items-center text-[10px] font-medium gap-2 ${
//                   activeId === chat.id ? "text-slate-500" : "text-gray-500"
//                 }`}
//               >
//                 <span>{chat.date}</span>
...
const ChatHistory = ({ onClose }) => {
  ...
  const conversations = [
    { id: "1", title: "hey", date: "Nov 28", msgCount: "10 msgs" },
    { id: "2", title: "New Conversation", date: "Sep 7", msgCount: "0 msgs" },
    {
      id: "3",
      title: "Getting Started with AI",
      date: "Jan 15",
  ...
  return (
    <div className="w-72 md:w-80 h-full bg-[#0b0f19] border-r border-gray-800 flex flex-col p-4 relative">
      {/* 🔹 Close button — works ONLY on PHONE */}
      <button
        onClick={() => {
          if (window.innerWidth < 768) onClose(); // close only in phone
        }}

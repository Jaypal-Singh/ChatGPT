...
const Stats = () => {
  const StatCard = ({ title, value, change, icon, gradient, shadowColor }) => (
    ...
    >
      <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${gradient} opacity-5 rounded-full blur-2xl -mr-10 -mt-10 pointer-events-none`}></div>

      <div className="flex justify-between items-start mb-2 z-10">
        <div>
          <span className="text-gray-400 text-sm font-medium block mb-1">{title}</span>
          <span className="text-3xl font-bold text-white tracking-tight">{value}</span>
        </div>
        <div className={`flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br ${gradient} shadow-lg shadow-gray-900/20`}>
      ...
      <div className="flex items-center mt-auto z-10">
        ...
        >
          <i className={`fas fa-arrow-${change.includes("+") ? "up" : "down"} text-[10px]`}></i>
          {change}
        </span>
      </div>
    </div>
